#############################################
# Correlaciones + Heatmap (Medellín)        #
#############################################
...
# ---- Paths ----
output_dir <- "C:/Users/danie/OneDrive/Escritorio/Natura/271025_Results_Med/output"
...
# Limpieza mínima (coherente con tu script)
...
pretty_labels <- function(x) {
  x %>%
    ...
    str_replace("^p28 importancia ", "Imp. ")           # compacta prefijo
}
...
p_heat_cont <- GGally::ggcorr(
  ...
  label = TRUE,
  label_round = 2,
  label_size = 2.8,            # números más pequeños
  layout.exp = 1
) +
  ggtitle("Correlaciones (Pearson) - Variables Continuas - Medellín") +
  theme(
    plot.title  = element_text(hjust = 0.5),
    axis.text.x = element_text(size = 9, angle = 45, hjust = 1, vjust = 1),
    axis.text.y = element_text(size = 9)
  ) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2))     # evita superposición

# Mostrar y guardar
print(p_heat_cont)
ggsave(
  filename = file.path(output_dir, "heatmap_correlaciones_continuas_medellin.png"),
  plot = p_heat_cont,
  width = 14, height = 10, dpi = 300
)

# ---- Función upper-tri tidy (evita el bug de rename/mutate) ----
upper_tri_tidy <- function(cor_mat) {
  as.data.frame(cor_mat) %>%
    rownames_to_column("var1") %>%
    pivot_longer(-var1, names_to = "var2", values_to = "r") %>%
    # nos quedamos con triángulo superior (orden alfabético)
    filter(var1 < var2) %>%
    mutate(abs_r = abs(r)) %>%
    arrange(desc(abs_r))
}
...
writexl::write_xlsx(
  ...
  path = file.path(output_dir, "correlaciones_medellin.xlsx")
)

# ---- Mensajes ----
message("✅ Heatmap: ", file.path(output_dir, "heatmap_correlaciones_continuas_medellin.png"))
message("✅ Excel:   ", file.path(output_dir, "correlaciones_medellin.xlsx"))
