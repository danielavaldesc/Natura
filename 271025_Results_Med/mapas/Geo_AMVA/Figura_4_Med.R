...
# -------------------------------------------------------------------
# 2) Estrato predominante por comuna (TU CÓDIGO)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 5) Pies: cuadrantes + COLORES/ORDEN FIJO (no se dañan)
# -------------------------------------------------------------------
colores_medio <- c(
  "Auto privado"        = "#C86A62",
  "Modo activo"         = "#D4B86A",
...
df_counts <- data %>%
  ...
  complete(motivo, cuadrante, medio = levels(data$medio), fill = list(n = 0)) %>%
  pivot_wider(names_from = medio, values_from = n, values_fill = 0)

cols_pie <- names(colores_medio)  # orden fijo
...
# -------------------------------------------------------------------
# 7) Metro: NOMBRES COMPLETOS + colores fijos + orden de leyenda
# -------------------------------------------------------------------
...
map.med.motivo <- ggplot() +
  ...
  scale_fill_manual(
    ...
    drop = FALSE,
    na.value = "#FFFFFF",
    na.translate = FALSE   # <- NO muestra NA
  ) +
  ...
  theme(
    ...
    # grid suave (si lo quieres sin grid, cambia a element_blank())
...
ggsave(
  plot = map.med.motivo,
  filename = "map.med_motivo_con_metro.png",
