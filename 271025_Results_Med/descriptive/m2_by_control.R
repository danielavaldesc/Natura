...
df_m2 <- dataset %>% dplyr::select(any_of(vars_m2$codigo))
...
# Ejecutar y exportar para cada variable de control
resultados_m2 <- lapply(names(control_vars_m2), function(ctrl) {
  ...
  out_path <- sprintf("output/m2/27102025_mod2_%s.xlsx", control_vars_m2[[ctrl]])
