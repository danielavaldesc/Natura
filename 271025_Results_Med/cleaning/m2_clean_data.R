...
dataset <- dataset %>%
  mutate(
    p17_modo_agregado = case_when(
      ...
      # Transporte público
      p17 %in% c("Bus, colectivo, buseta", "Metro", "Metroplus", "Metro Cable", "Tranvía") ~ "Transporte público",
...
dataset <- dataset %>%
  mutate(
    cilindraje_auto_agregado = case_when(
      p17 %in% c("Automóvil", "Campero/ Camioneta (SUV)") ~ case_when(
        ...
        p17_v2_1 == "3000 cc o más" ~ "3000 cc o más",
