...
if (is.null(var_estrato)) stop("No pude detectar la variable de ESTRATO.")
...
out_dir  <- file.path("1.analisis_estadistico","Output_by_estrato")
...
res <- run_descriptivo(
  ...
  group_vars        = c(var_estrato),    # <-- por estrato
  out_xlsx          = file.path(out_dir, "descriptivo_por_estrato.xlsx"),
  out_tex_dir       = file.path(out_dir, "tex")
)

message("OK: descriptivo por ESTRATO exportado a: ", out_dir)
